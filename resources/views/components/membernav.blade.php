          ...
          <nav id="navbar" class="navbar">
              <ul>
                  ...
                  @auth
                      ...
                      <li><a style="font-size: 20px; color: rgb(212, 0, 0);"  class="nav-link scrollto"
                              href="{{ url('member_logout') }}">تسجيل خروج</a></li>

                      <li class="dropdown">
                          <a href="{{ url("union/showservice/$union->id") }}" class="scrollto">
                              <span style="font-size: 20px;" class="nav-link scrollto">الخدمات</span>
                              <i class="bibi-chevron-down">
                              </i>
                          </a>
                          <ul>
                              @foreach ($servicess as $service)
                                  <li>
                                      <a href="{{ url("union/serviceform/{$service->id}") }}"> نقابة
                                          {{ $service->namear }}</a>
                                  </li>
                              @endforeach
                          </ul>
